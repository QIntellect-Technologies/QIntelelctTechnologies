        ...
        <div className="flex flex-col md:flex-row gap-8 mb-20 items-center justify-between sticky top-24 z-30 py-4 bg-white/80 backdrop-blur-xl">
          <div className="relative w-full md:w-96 group">
            ...
            <input 
              ...
              className="w-full pl-16 pr-8 py-5 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-600 transition-all text-sm font-medium"
            />
          </div>
          ...
          <AnimatePresence mode='popLayout'>
            {filteredBlogs.map((post, i) => (
              <motion.div
                ...
                className="group flex flex-col h-full bg-white glass-card rounded-[3.5rem] border border-slate-100 overflow-hidden shadow-xl hover:shadow-4xl hover:-translate-y-4 transition-all duration-700"
              >
                <Link to={`/blog/${post.id}`} className="block h-72 overflow-hidden relative">
                  ...
                  />
                  <div className="absolute top-6 left-6 px-4 py-2 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-xl">
                    {post.category}
                  </div>
                ...
                <div className="p-12 flex flex-col flex-grow space-y-8">
                  <div className="flex items-center space-x-6 text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                  ...
                  <h3 className="text-2xl font-medium text-slate-900 font-heading tracking-tight leading-snug group-hover:text-blue-600 transition-colors">
                    {post.title}
                  </h3>

                  <p className="text-slate-500 text-sm font-light leading-relaxed flex-grow line-clamp-3">
                    {post.excerpt}
                  </p>

                  <div className="flex flex-wrap gap-2 pt-4">
                     {post.tags.slice(0, 2).map(tag => (
                       <span key={tag} className="text-[8px] font-black uppercase tracking-widest text-slate-400 flex items-center bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
                         <Tag className="w-2.5 h-2.5 mr-1.5 text-blue-600" /> {tag}
                       </span>
                     ))}
                  </div>
                  ...
                  <Link 
                    to={`/blog/${post.id}`} 
                    className="inline-flex items-center justify-between w-full p-6 rounded-3xl bg-slate-900 text-white hover:bg-blue-600 transition-all font-black text-[10px] uppercase tracking-widest relative z-10 shadow-2xl mt-4"
                  >
                    Open Blueprint
