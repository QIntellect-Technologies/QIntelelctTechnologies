           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {SERVICES.map((s, i) => (
                <motion.div 
                  ...
                  className="group p-6 bg-white glass-card rounded-2xl border border-white hover:border-blue-300 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-700 flex flex-col relative overflow-hidden"
                >
                   ...
                   <div className="w-10 h-10 bg-blue-600 text-white rounded-xl flex items-center justify-center mb-6 group-hover:shadow-2xl group-hover:rotate-6 transition-all shadow-xl relative z-10">
                      <Code className="w-5 h-5" />
                   </div>
                   
                   <h4 className="text-xl font-medium text-slate-900 mb-3 font-heading tracking-tight relative z-10">{s.title}</h4>
                   <p className="text-slate-500 font-light leading-relaxed text-xs mb-6 flex-grow relative z-10">
                     {s.shortDescription}
                   </p>
                   
                   <ul className="space-y-2 mb-6 relative z-10">
                      {s.features.slice(0, 3).map((f, idx) => (
                        <li key={idx} className="flex items-center text-[8px] text-slate-400 font-bold uppercase tracking-[0.2em]">
                           <div className="w-1.5 h-1.5 rounded-full bg-blue-600 mr-3 shadow-sm" />
                           {f}
                        </li>
                      ))}
                   </ul>
