...
const TestimonialSection: React.FC = () => {
  ...
  return (
    <section className="py-32 bg-[#F8F9FB] relative overflow-hidden">
      {/* Background Neural Pattern exactly like image */}
      <div className="absolute inset-0 opacity-[0.4] pointer-events-none">
        <svg className="w-full h-full" viewBox="0 0 1200 800" fill="none">
          <circle cx="100" cy="150" r="3" fill="#E11D48" />
          <circle cx="300" cy="100" r="3" fill="#E11D48" />
          <circle cx="500" cy="200" r="3" fill="#E11D48" />
          <circle cx="800" cy="150" r="3" fill="#E11D48" />
          <circle cx="200" cy="400" r="3" fill="#E11D48" />
          <circle cx="600" cy="500" r="3" fill="#E11D48" />
          <circle cx="950" cy="450" r="3" fill="#E11D48" />
          <line x1="100" y1="150" x2="300" y2="100" stroke="#E2E8F0" strokeWidth="1" />
          <line x1="300" y1="100" x2="500" y2="200" stroke="#E2E8F0" strokeWidth="1" />
          <line x1="500" y1="200" x2="800" y2="150" stroke="#E2E8F0" strokeWidth="1" />
          <line x1="200" y1="400" x2="600" y2="500" stroke="#E2E8F0" strokeWidth="1" />
          <line x1="600" y1="500" x2="950" y2="450" stroke="#E2E8F0" strokeWidth="1" />
          <line x1="100" y1="150" x2="200" y2="400" stroke="#E2E8F0" strokeWidth="1" />
          <line x1="500" y1="200" x2="600" y2="500" stroke="#E2E8F0" strokeWidth="1" />
        </svg>
      </div>

      <div className="container mx-auto px-4 md:px-12 relative z-10">
        {/* Badge and Title - Reduced gap with mb-6 */}
        <div className="mb-6 ml-[2%]">
           <div className="flex items-center space-x-2 text-[#E11D48] mb-3">
              <div className="w-2 h-2 rounded-full bg-current" />
              <span className="text-[12px] font-bold uppercase tracking-[0.4em]">Testimonial</span>
           </div>
           <h2 className="text-4xl md:text-5xl font-bold text-slate-900 leading-[1.1] font-heading max-w-3xl">
        ...
        <div className="relative flex flex-col lg:flex-row items-stretch justify-between gap-8">
           {/* Card Container - Increased height to match larger image */}
           <div className="w-full lg:w-2/3 relative flex items-center h-[560px] overflow-hidden">
              <div className="flex space-x-8 overflow-visible self-start pt-4">
                 {testimonials.map((t, i) => (
                    <motion.div
                       key={i}
                       animate={{ 
                          x: -activeIndex * (440 + 32),
                          opacity: i === activeIndex ? 1 : (i > activeIndex ? 0.3 : 0),
                          scale: i === activeIndex ? 1 : 0.95,
                       }}
                       transition={{ duration: 0.7, ease: [0.32, 0.72, 0, 1] }}
                       className="flex-shrink-0 w-[440px] bg-white p-10 rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.03)] border border-slate-100 flex flex-col justify-between h-[480px]"
                    >
                       <div className="space-y-6">
                          <div className="flex space-x-1">
                             {[1,2,3,4,5].map(s => <StarIcon key={s} className="w-3.5 h-3.5 text-[#F97316] fill-[#F97316]" />)}
                          </div>
                       ...
                       <div className="flex items-center justify-between mt-auto">
                          <div className="flex items-center space-x-4">
                             <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full ring-4 ring-slate-50 shadow-sm" />
                             <div>
                                <h4 className="text-base font-bold text-slate-900">{t.name}</h4>
                                <p className="text-[11px] text-slate-400 font-medium tracking-wide uppercase">{t.role}</p>
                             </div>
                          </div>
                          <Quote className="w-10 h-10 text-[#F97316] opacity-10" />
                       </div>
                    </motion.div>
                 ))}
              </div>

               {/* Navigation Buttons - Positioned to the bottom left of cards */}
              <div className="absolute bottom-4 left-0 flex space-x-4">
                  <button onClick={prev} className="w-11 h-11 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-900 hover:text-white transition-all bg-white shadow-sm">
                    <ArrowRight className="w-4 h-4 rotate-180" />
                  </button>
                  <button onClick={next} className="w-11 h-11 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-900 hover:text-white transition-all bg-white shadow-sm">
                    <ArrowRight className="w-4 h-4" />
                  </button>
              </div>
           </div>

           {/* AI Image - Increased height to 560px as requested */}
           <div className="hidden lg:block w-[420px] h-[560px] relative">
              <div className="absolute inset-0 bg-white rounded-[4rem] shadow-xl overflow-hidden p-4 border border-slate-50">
