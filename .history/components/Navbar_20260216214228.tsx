...
const StatusTypewriter: React.FC = () => {
...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'py-2' : 'py-6'}`}>
      <div className="container mx-auto px-4 md:px-8">
        <div className={`relative transition-all duration-300 rounded-2xl px-6 py-3 flex items-center justify-between ${scrolled ? 'bg-white/80 backdrop-blur-md shadow-xl shadow-slate-200/40 border border-slate-100' : 'bg-transparent'}`}>
          <div className="flex items-center space-x-12">
            <Link to="/" className="flex items-center space-x-3 group">
              <div className="w-10 h-10 bg-slate-900 rounded-lg flex items-center justify-center shadow-md group-hover:bg-blue-600 transition-colors">
                <span className="text-white font-bold text-xl uppercase italic">Q</span>
              </div>
              <div className="flex flex-col">
                <span className="text-lg font-black text-slate-900 tracking-tighter leading-none uppercase italic">Intellect</span>
                <span className="text-[8px] font-black text-blue-600 tracking-[0.2em] uppercase mt-0.5">Premium Engineering</span>
              </div>
            </Link>
