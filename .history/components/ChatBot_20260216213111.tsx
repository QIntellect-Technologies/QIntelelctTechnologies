  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100]">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="mb-6 w-96 md:w-[450px] h-[700px] flex flex-col rounded-3xl shadow-4xl overflow-hidden bg-slate-900 border border-slate-800 text-white"
          >
            ...
            <div ref={scrollRef} className="flex-1 p-8 overflow-y-auto space-y-6 bg-slate-950/50" style={{ transform: "translateZ(10px)" }}>
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`flex items-start max-w-[85%] space-x-4 ${msg.role === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                    ...
                    <div className={`p-5 rounded-3xl text-xs leading-relaxed ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-slate-900 border border-slate-800 text-slate-300 rounded-tl-none shadow-xl'}`}>
                      {msg.text}
                    </div>
                  </div>
                </div>
              ...
              {isThinking && (
                <div className="flex justify-start">
                  <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-3xl rounded-tl-none flex items-center space-x-3">
      ...
      <motion.button
        ...
        className="w-20 h-20 rounded-[2rem] bg-blue-600 text-white flex items-center justify-center shadow-[0_20px_50px_-10px_rgba(37,99,235,0.6)] hover:bg-blue-700 transition-all z-50 relative group"
      >
