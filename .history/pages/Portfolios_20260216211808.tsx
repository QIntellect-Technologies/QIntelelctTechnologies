  ...
  return (
    <div ref={containerRef} className="pt-48 pb-24 bg-white font-light selection:bg-blue-100 selection:text-blue-700">
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 mb-24">
          {categories.map((cat) => (
            <button
              ...
              className={`px-6 py-2 rounded-3xl text-[10px] font-black uppercase tracking-widest transition-all border ${
                activeCategory === cat 
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project, i) => (
              ...
              >
                ...
                <div className="relative rounded-[4rem] overflow-hidden aspect-[16/10] shadow-4xl bg-slate-900 border border-slate-100">
                   <img 
                    src={project.image} 
                    alt={project.title} 
                  ...
                  {/* Floating Tech Tag */}
                  <div className="absolute top-10 left-10 p-5 bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/60 flex items-center space-x-4">
                  ...
                  <div className="absolute bottom-10 right-10 flex space-x-4">
                     {project.metrics.map((m, idx) => (
                       <div key={idx} className="p-6 bg-slate-900/80 backdrop-blur-xl rounded-[2.5rem] border border-white/10 text-white shadow-2xl">
                          <div className="text-3xl font-heading font-medium tracking-tighter text-blue-400">{m.value}</div>
                ...
                <div className="px-6 space-y-8">
                   <div className="flex justify-between items-start">
                      <div className="space-y-4">
                         <span className="text-[11px] font-black text-blue-600 uppercase tracking-[0.5em]">{project.client}</span>
                         <h2 className="text-4xl md:text-5xl font-medium text-slate-900 font-heading tracking-tighter leading-none">{project.title}</h2>
                      </div>
                      <button className="w-16 h-16 rounded-full border border-slate-100 flex items-center justify-center text-slate-400 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all shadow-sm">
                   ...
                   <div className="flex flex-wrap gap-4 pt-4">
                      {project.stack.map(s => (
                        <div key={s} className="px-5 py-2 bg-slate-50 rounded-xl text-[10px] font-black text-slate-400 uppercase tracking-widest border border-slate-100 flex items-center">
                           <Terminal className="w-3 h-3 mr-3 text-blue-600" />
                           {s}
                        </div>
                      ))}
                   </div>
                   ...
                   <div className="pt-8 border-t border-slate-100">
                      <button className="inline-flex items-center text-[11px] font-black uppercase tracking-[0.4em] text-slate-900 hover:text-blue-600 transition-colors">
                        View System Blueprint <ChevronRight className="w-5 h-5 ml-4" />
                      </button>
                   </div>
                </div>
              </motion.div>
        ...
        {/* Bottom CTA Section */}
        <section className="mt-48 py-32 bg-slate-900 rounded-[5rem] text-white relative overflow-hidden">
           ...
           <div className="container mx-auto px-4 text-center relative z-10 space-y-16">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-10 pt-8">
                 <button className="px-16 py-8 bg-blue-600 text-white rounded-[3rem] font-black hover:bg-blue-700 transition-all shadow-4xl tracking-widest uppercase text-xs">Request Strategy Session</button>
                 <button className="px-16 py-8 border-2 border-white/10 text-white rounded-[3rem] font-black hover:bg-white hover:text-slate-900 transition-all tracking-widest uppercase text-xs">Technical Docs</button>
              </div>
           </div>
        </section>
      </div>
