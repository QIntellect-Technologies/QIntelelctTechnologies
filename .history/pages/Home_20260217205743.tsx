...
const WhyChooseUs: React.FC = () => {
  const features = [
    ...
    },
    {
      ...
      desc: "Our team includes certified Dynamics 365 consultants, AI researchers & Engineers, EDI specialists, and full-stack developers with deep industry expertise.",
      icon: Users,
    ...
    <section className="py-32 bg-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="relative max-w-7xl mx-auto">
          {/* Desktop Circular Layout & Connecting Lines */}
          <div className="hidden lg:block absolute inset-0 pointer-events-none">
             <svg className="w-full h-full" viewBox="0 0 900 900" fill="none">
                {/* Connecting Lines from center to each card */}
                {features.map((f, i) => {
                   const angle = (i * 360 / features.length) - 90; // Start from top, distribute evenly
                   const radian = (angle * Math.PI) / 180;
                   const centerX = 450;
                   const centerY = 450;
                   const radius = 342; // 38% of 900px
          ...
          <div className="relative">
            ...
            <div className="lg:hidden grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
              {features.map((f) => (
                <motion.div
                  ...
                  className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 group cursor-pointer"
                >
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-slate-950 flex items-center justify-center text-white group-hover:bg-blue-600 transition-all duration-500">
                      <f.icon className="w-6 h-6" />
                    </div>
                    <div className="space-y-2">
                      <h3 className="text-lg font-medium font-heading tracking-tight group-hover:text-blue-600 transition-colors">{f.title}</h3>
            ...
            <div className="hidden lg:block relative h-[900px] w-[900px] mx-auto">
              {/* Circular Card Positions */}
              {features.map((f, i) => {
                const angle = (i * 360 / features.length) - 90; // Start from top
                const radian = (angle * Math.PI) / 180;
                const centerX = 50; // 50% from left
                const centerY = 50; // 50% from top
                const x = centerX + Math.cos(radian) * 38; // 38% radius for better balance
                const y = centerY + Math.sin(radian) * 38;
                ...
                return (
                  <motion.div
                    ...
                    className="w-56 bg-white p-5 rounded-2xl border border-slate-100 shadow-lg hover:shadow-2xl hover:border-blue-100 transition-all duration-500 group cursor-pointer z-20"
                  >
                    <div className="flex flex-col items-center text-center space-y-3">
                      <div className={`w-12 h-12 rounded-xl bg-slate-950 flex items-center justify-center text-white group-hover:bg-blue-600 group-hover:rotate-12 transition-all duration-500 shadow-lg`}>
                        <f.icon className="w-6 h-6" />
                      </div>
                      <div className="space-y-2">
                        <h3 className="text-base font-semibold font-heading tracking-tight group-hover:text-blue-600 transition-colors leading-tight">{f.title}</h3>
              ...
              {/* Center Hub - Positioned at the center of the circle */}
              <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30">
                <motion.div 
                  initial={{ opacity: 0, scale: 0.8 }}
                  whileInView={{ opacity: 1, scale: 1 }}
                  className="w-64 h-64 rounded-[4rem] bg-gradient-to-br from-blue-600 to-indigo-700 p-2 shadow-[0_0_60px_rgba(37,99,235,0.3)] flex items-center justify-center relative group"
                >
                  <div className="absolute inset-0 bg-blue-600 rounded-[4rem] blur-3xl opacity-20 group-hover:opacity-40 transition-opacity animate-pulse" />
                  <div className="w-full h-full bg-white rounded-[3.8rem] flex flex-col items-center justify-center p-8 text-center space-y-4 relative overflow-hidden">
                    ...
                    <motion.div 
                      ...
                      className="w-20 h-20 bg-slate-900 rounded-3xl flex items-center justify-center relative z-10"
                    >
                      <Rocket className="w-10 h-10 text-white" />
                    </motion.div>
                    <div className="relative z-10">
                      <span className="text-xs font-black uppercase tracking-widest text-blue-600">QIntellect</span>
                      <h4 className="text-2xl font-bold font-heading text-slate-900 mt-1">Q-Engine</h4>
                      <p className="text-xs text-slate-600 mt-1">Unified Technology Ecosystem</p>
                    </div>
                  </div>
                </motion.div>
              </div>
        ...
        <div className="hidden lg:flex flex-col items-end absolute right-12 top-1/2 -translate-y-1/2 space-y-20 z-20">
          <div className="text-right space-y-4">
            <h4 className="text-sm font-bold text-slate-900 leading-snug font-heading uppercase tracking-tighter">
              AI, ERP <br /> & Enterprise <br /> Solutions.
            </h4>
          ...
          <div className="flex flex-col space-y-16 max-w-7xl">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-end">
               ...
               <div className="flex flex-col space-y-10">
                  ...
                  >
                    From Artificial Intelligence and Customized Chatbots to Dynamics 365, ERP, EDI, and Web Development — QIntellect delivers end-to-end enterprise technology solutions that transform how you operate.
                  </motion.p>
