...
const TestimonialSection: React.FC = () => {
  ...
  return (
    <section className="py-48 bg-[#FAFAFB] relative overflow-hidden">
      {/* High-Fidelity Geometric Background (Matching Image) */}
      <div className="absolute inset-0 opacity-[0.08] pointer-events-none">
         <svg className="w-full h-full" viewBox="0 0 1200 800" fill="none">
            <path d="M100 200L300 100L500 150L800 50L1100 120" stroke="#2563eb" strokeWidth="0.5" />
            <path d="M50 400L250 350L450 450L750 400L1150 500" stroke="#2563eb" strokeWidth="0.5" />
            <path d="M150 600L400 550L650 650L950 550L1200 620" stroke="#2563eb" strokeWidth="0.5" />
            <path d="M300 100L250 350" stroke="#2563eb" strokeWidth="0.5" />
            <path d="M500 150L450 450" stroke="#2563eb" strokeWidth="0.5" />
            <path d="M750 400L650 650" stroke="#2563eb" strokeWidth="0.5" />
            {[100,300,500,800,1100,50,250,450,750,1150,150,400,650,950].map((x, i) => {
               const ys = [200,100,150,50,120,400,350,450,400,500,600,550,650,550];
               return <circle key={i} cx={x} cy={ys[i]} r="2.5" fill="#2563eb" />;
            })}
         </svg>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-12 relative z-10">
        <div className="flex flex-col space-y-24">
           ...
           {/* Section Header - Exactly like attachment */}
           <div className="space-y-6 max-w-4xl ml-[5%]">
              <div className="flex items-center space-x-3 text-blue-600">
                 <div className="w-2.5 h-2.5 rounded-full bg-current shadow-[0_0_12px_rgba(37,99,235,0.6)]" />
                 <span className="text-[12px] font-black uppercase tracking-[0.5em]">Testimonial</span>
              </div>
              <h2 className="text-5xl md:text-8xl font-medium text-slate-900 leading-[0.95] tracking-tighter font-heading uppercase italic">
                Hear what our about customer <br /> 
                say about our <span className="text-blue-600">AI agency</span>
              </h2>
           </div>

           <div className="relative grid grid-cols-1 lg:grid-cols-12 items-center gap-0">
              {/* Testimonial Multi-Card Slider Area */}
              <div className="lg:col-span-9 order-2 lg:order-1 relative h-[600px] flex items-center">
                 <div className="flex space-x-12 overflow-visible">
                    {testimonials.map((t, i) => (
                      <motion.div
                        ...
                        className="flex-shrink-0 w-[480px] bg-white p-14 rounded-[3.5rem] shadow-[0_50px_100px_rgba(0,0,0,0.04)] border border-slate-50 flex flex-col justify-between"
                      >
                         <div className="space-y-10">
                            <div className="flex space-x-1.5">
                               {[1,2,3,4,5].map(s => <StarIcon key={s} className="w-4 h-4 text-[#FF7A00] fill-[#FF7A00]" />)}
                            </div>
                            <p className="text-xl md:text-2xl text-slate-600 font-light leading-snug tracking-tight">
                               "{t.text}"
                            </p>
                         </div>
                         ...
                         <div className="flex items-center justify-between mt-12">
                            <div className="flex items-center space-x-5">
                               <img src={t.avatar} alt={t.name} className="w-16 h-16 rounded-full border-4 border-slate-50 shadow-md" />
                               <div>
                                  <h4 className="text-xl font-bold text-slate-900 tracking-tight">{t.name}</h4>
                                  <p className="text-sm text-slate-400 font-medium uppercase tracking-widest">{t.role}</p>
                               </div>
                            </div>
                            <Quote className="w-16 h-16 text-blue-600/10" />
                         </div>
                      </motion.div>
                    ))}
                 </div>

                 {/* Navigation at bottom of cards */}
                 <div className="absolute -bottom-12 left-0 flex space-x-6">
                    <button onClick={prev} className="w-16 h-16 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-900 hover:text-white transition-all shadow-sm group">
                       <ArrowRight className="w-6 h-6 rotate-180 group-hover:-translate-x-1 transition-transform" />
                    </button>
                    <button onClick={next} className="w-16 h-16 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-900 hover:text-white transition-all shadow-sm group">
                       <ArrowRight className="w-6 h-6 group-hover:translate-x-1 transition-transform" />
                    </button>
                 </div>
              </div>

              {/* High-Performance AI Image Overlay (Precise Matching) */}
              <div className="lg:col-span-5 order-1 lg:order-2 lg:-ml-40 relative z-0">
                 <motion.div 
                   initial={{ opacity: 0, x: 100 }}
                   whileInView={{ opacity: 1, x: 0 }}
                   transition={{ duration: 1 }}
                   className="rounded-[4.5rem] overflow-hidden shadow-[0_60px_120px_rgba(37,99,235,0.2)] aspect-[3/4] group border-[15px] border-white relative bg-slate-900"
                 >
                    <img 
                      src="https://images.unsplash.com/photo-1620712943543-bcc4628c940c?q=80&w=2000&auto=format&fit=crop" 
                      alt="AI Persona" 
                      className="w-full h-full object-cover grayscale-[15%] group-hover:grayscale-0 transition-all duration-[2s] scale-110 group-hover:scale-100"
                    />
